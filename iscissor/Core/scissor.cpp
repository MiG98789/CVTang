...
void Scissor::PathTree(Mat& tree, int nodes)
{
    ...
    for(int i = 0; i < nodes && i < explore.size(); i++)
    {
        ...
        for(int j = 0; j < size - 1; j++)
        {
            ...
            int ox = p1.x - p.x;
            int oy = p1.y - p.y;
            tree.at<Vec3b>(3*p.y+1+oy, 3*p.x+1+ox) = Vec3b(0, 255, 255);
            tree.at<Vec3b>(3*p.y+1, 3*p.x+1) = Vec3b(255, 255, 0);
            tree.at<Vec3b>(3*p1.y+1-oy, 3*p1.x+1-ox) = Vec3b( 0, 127, 127);
        }
    }
}
...
void Scissor::OnClick()
{
    ...
    if (path.seeds.size() > 1 && path.seeds[path.seeds.size() - 1] == path.seeds[path.seeds.size() - 2])
    {
        path.seeds.pop_back();
        return;
    }
